...
public class ManagerController implements ActionListener, DocumentListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Add Vehicle")) {
			...
			char[] oldPassword = managerFrame.getChangePasswordPanel()
					.getCurrentPass().getPassword();
			...
			if (inDatabase) {
				char[] newPassword = managerFrame.getChangePasswordPanel()
						.getNewPass().getPassword();
				String newPass = new String(newPassword);
				char[] confirmPassword = managerFrame.getChangePasswordPanel()
						.getConfirmNewPass().getPassword();
				...
				if (Arrays.equals(newPassword, confirmPassword)) {
					...
					if (setPassword.setNewPassword(newPass)) {
						managerFrame.getChangePasswordPanel().getWrongInput()
			...
			managerFrame.getUpdateProfileForManager().getTextName()
					.setText(name);
			managerFrame.getUpdateProfileForManager().getTextPhone()
					.setText(phone);
			managerFrame.getUpdateProfileForManager().getTextAddress()
					.setText(address);
			managerFrame.getUpdateProfileForManager().getTxtEmail()
					.setText(email);
			...
			managerFrame.getClerkHome().setLocation(
					(Toolkit.getDefaultToolkit().getScreenSize().width) / 2
							- managerFrame.getClerkHome().getWidth() / 2,
					(Toolkit.getDefaultToolkit().getScreenSize().height) / 2
							- managerFrame.getClerkHome().getHeight() / 2);
			...
			managerFrame.getManageRatesPanel().getRentalratePanel()
					.revalidate();
			...
			int row = managerFrame.getSearchVehicleListPanel().getSearchtable()
					.getSelectedRow();
			if (row != -1) {
				managerFrame.getSellingVehicleDialog().setRegNumberTxt(
						managerFrame.getSearchVehicleListPanel()
								.getSearchtable().getModel().getValueAt(row, 0)
								.toString());
				managerFrame.getSellingVehicleDialog().setCategoryCombox(
						managerFrame.getSearchVehicleListPanel()
								.getSearchtable().getModel().getValueAt(row, 1)
								.toString());
				managerFrame.getSellingVehicleDialog().setTypeCombox(
						managerFrame.getSearchVehicleListPanel()
								.getSearchtable().getModel().getValueAt(row, 2)
								.toString());
				managerFrame.getSellingVehicleDialog().setBrandTxt(
						(managerFrame.getSearchVehicleListPanel()
								.getSearchtable().getModel().getValueAt(row, 3)
								.toString()));
				managerFrame.getSellingVehicleDialog()
						.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
				managerFrame.getSellingVehicleDialog().setLocation(
			...
			sellVehicleVO.setRegNo(managerFrame.getSellingVehicleDialog()
					.getRegNumberTxt());
			sellVehicleVO.setPrice(managerFrame.getSellingVehicleDialog()
					.getPrice());
			...
			int row = managerFrame.getSellVehicleListPanel().getSellTable()
					.getSelectedRow();
			if (row != -1) {
				sellVehicleVO.setRegNo(managerFrame.getSellVehicleListPanel()
						.getSellTable().getModel().getValueAt(row, 0)
						.toString());
			...
			VehicleVO vehicleVO = new VehicleVO();
			vehicleVO.setRegNo(managerFrame.getEditVehicleInfoDialog()
					.getAddVehiclePanel().getRegNumberTxt());

			vehicleVO.setCategory((String) managerFrame
					.getEditVehicleInfoDialog().getAddVehiclePanel()
					.getCategoryCombox().getSelectedItem());
			vehicleVO.setType((String) managerFrame.getEditVehicleInfoDialog()
					.getAddVehiclePanel().getTypeCombox().getSelectedItem());
			vehicleVO.setBrand(managerFrame.getEditVehicleInfoDialog()
					.getAddVehiclePanel().getBrandTxt());
			Date purchaseDate = managerFrame.getEditVehicleInfoDialog()
					.getAddVehiclePanel().getCalendar().getDate();
			java.sql.Date sqlDate = new java.sql.Date(purchaseDate.getTime());
			vehicleVO.setPurchaseDate(sqlDate);
			...
			if (row != -1) {
				managerFrame
						.getEditVehicleInfoDialog()
						.getAddVehiclePanel()
						.setRegNumberTxt(
								(String) managerFrame
										.getSearchVehicleListPanel()
										.getSearchtable().getModel()
				...
				managerFrame.getEditVehicleInfoDialog().setLocation(
						(Toolkit.getDefaultToolkit().getScreenSize().width)
								/ 2
								- managerFrame.getEditVehicleInfoDialog()
										.getWidth() / 2,
						(Toolkit.getDefaultToolkit().getScreenSize().height)
								/ 2
								- managerFrame.getEditVehicleInfoDialog()
			...
			int row = managerFrame.getSellVehicleListPanel().getSellTable()
					.getSelectedRow();
			if (row != -1) {
				managerFrame.getEditForSalePriceDialog().setRegisterNumber(
						(String) managerFrame.getSellVehicleListPanel()
								.getSellTable().getModel().getValueAt(row, 0)
								.toString());
				managerFrame.getEditForSalePriceDialog().setPrice(
						(String) managerFrame.getSellVehicleListPanel()
								.getSellTable().getModel().getValueAt(row, 1)
								.toString());
				managerFrame.getEditForSalePriceDialog()
						.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
			...
			SellVehicleVO sellVehicleVO = new SellVehicleVO();
			sellVehicleVO.setRegNo(managerFrame.getEditForSalePriceDialog()
					.getRegisterNumber());
			sellVehicleVO.setPrice(managerFrame.getEditForSalePriceDialog()
					.getPrice());
			...
			int row = managerFrame.getManageRatesPanel().getRentalRateTable()
					.getSelectedRow();
			SuperRentRentalRate superRentRentalRate = new SuperRentRentalRate();
			if (row != -1) {
				superRentRentalRate.setCategory(managerFrame
						.getManageRatesPanel().getRentalRateTable().getModel()
						.getValueAt(row, 1).toString());
				superRentRentalRate.setType(managerFrame.getManageRatesPanel()
						.getRentalRateTable().getModel().getValueAt(row, 2)
						.toString());
				superRentRentalRate.setDailyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getRentalRateTable().getModel()
								.getValueAt(row, 3).toString()));
				superRentRentalRate.setWeeklyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getRentalRateTable().getModel()
								.getValueAt(row, 4).toString()));
				superRentRentalRate.setHourlyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getRentalRateTable().getModel()
								.getValueAt(row, 5).toString()));
				superRentRentalRate.setPerKMRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getRentalRateTable().getModel()
								.getValueAt(row, 6).toString()));
				superRentRentalRate.setMileageLimit(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getRentalRateTable().getModel()
								.getValueAt(row, 7).toString()));
				boolean result = managerDao
						.updateRentalRate(superRentRentalRate);
			...
			int row = managerFrame.getManageRatesPanel().getInsuranceTable()
					.getSelectedRow();
			SuperRentInsuranceRate superRentInsuranceRate = new SuperRentInsuranceRate();
			if (row != -1) {
				superRentInsuranceRate.setCategory(managerFrame
						.getManageRatesPanel().getInsuranceTable().getModel()
						.getValueAt(row, 1).toString());
				superRentInsuranceRate.setType(managerFrame
						.getManageRatesPanel().getInsuranceTable().getModel()
						.getValueAt(row, 2).toString());
				superRentInsuranceRate.setDailyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getInsuranceTable().getModel()
								.getValueAt(row, 3).toString()));
				superRentInsuranceRate.setWeeklyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getInsuranceTable().getModel()
								.getValueAt(row, 4).toString()));
				superRentInsuranceRate.setHourlyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getInsuranceTable().getModel()
								.getValueAt(row, 5).toString()));
				// superRentInsuranceRate.setPerKMRate(Double.parseDouble(managerFrame.getManageRatesPanel().getInsuranceTable().getModel().getValueAt(row,
				// 6).toString()));
				// superRentInsuranceRate.setMileageLimit(Double.parseDouble(managerFrame.getManageRatesPanel().getInsuranceTable().getModel().getValueAt(row,
				// 7).toString()));
				boolean result = managerDao
						.updateInsuranceRate(superRentInsuranceRate);
			...
			int row = managerFrame.getManageRatesPanel()
					.getAdditionalEquipmentTable().getSelectedRow();
			AdditionalEquipment additionalEquipment = new AdditionalEquipment();
			if (row != -1) {
				additionalEquipment.setCategory(managerFrame
						.getManageRatesPanel().getAdditionalEquipmentTable()
						.getModel().getValueAt(row, 2).toString());

				additionalEquipment.setDailyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getAdditionalEquipmentTable().getModel()
								.getValueAt(row, 4).toString()));

				additionalEquipment.setHourlyRate(Double
						.parseDouble(managerFrame.getManageRatesPanel()
								.getAdditionalEquipmentTable().getModel()
								.getValueAt(row, 3).toString()));

				boolean result = managerDao
						.updateAddnEquipRate(additionalEquipment);
			...
			managerFrame.getUpdateProfileForManager().getTextPhone()
					.setForeground(Color.black);
			managerFrame.getUpdateProfileForManager().getUpdateInfo()
					.setText("");

			// check if user enters text in name text area
			if (managerFrame.getUpdateProfileForManager().getTextName()
					.getText() != null
					&& managerFrame.getUpdateProfileForManager().getTextName()
							.getText().trim().length() != 0) {
				// check if text area is "Update Success" to prevent updating
				if (!managerFrame.getUpdateProfileForManager().getTextName()
						.getText().equals("Update Success")) {
					UpdateProfileDAO updateNameDAO = new UpdateProfileDAO();
					if (updateNameDAO.updateName(managerFrame
							.getUpdateProfileForManager().getTextName()
							.getText())) {
						nameSuccess = 1;
						updateInfosString = updateInfosString
								+ "Update name success<br>";
					} else {
						nameSuccess = 0;
						updateInfosString = updateInfosString
								+ "Update name fail<br>";
			...
			if (managerFrame.getUpdateProfileForManager().getTextPhone()
					.getText() != null
					&& managerFrame.getUpdateProfileForManager().getTextPhone()
							.getText().trim().length() != 0) {
				...
				if (!managerFrame.getUpdateProfileForManager().getTextPhone()
					...
					String newPhoneNumber = managerFrame
							.getUpdateProfileForManager().getTextPhone()
							.getText();
					if (updatePhoneDAO.updatePhoneNumber(newPhoneNumber)) {
						phoneSuccess = 1;
						updateInfosString = updateInfosString
								+ "Update phone number success<br>";

					} else {
						phoneSuccess = 0;
						updateInfosString = updateInfosString
								+ "phone number is not vaild<br>";
					}
				} else {
					managerFrame.getUpdateProfileForManager().getTextPhone()
							.setForeground(Color.black);
			...
			if (managerFrame.getUpdateProfileForManager().getTextAddress()
					.getText() != null
					&& managerFrame.getUpdateProfileForManager()
							.getTextAddress().getText().trim().length() != 0) {

				// check if text area is "Update Success" to prevent updating
				if (!managerFrame.getUpdateProfileForManager().getTextAddress()
						.getText().equals("Update Success")) {
					UpdateProfileDAO updateAddressDAO = new UpdateProfileDAO();
					String newAddress = managerFrame
							.getUpdateProfileForManager().getTextAddress()
							.getText();
					if (updateAddressDAO.updateAddress(newAddress)) {
						addressSucess = 1;
						updateInfosString = updateInfosString
								+ "Update address success<br>";
					} else {
						addressSucess = 0;
						updateInfosString = updateInfosString
								+ "Address is not valid<br>";
					}
				} else {
					managerFrame.getUpdateProfileForManager().getTextAddress()
							.setForeground(Color.black);
			...
			if (managerFrame.getUpdateProfileForManager().getTxtEmail()
					.getText() != null
					&& managerFrame.getUpdateProfileForManager().getTxtEmail()
							.getText().trim().length() != 0) {
				...
				if (!managerFrame.getUpdateProfileForManager().getTxtEmail()
					...
					String email = managerFrame.getUpdateProfileForManager()
							.getTxtEmail().getText();
					if (updateEmailDAO.updatemail(email)) {
						emailSuccess = 1;
						updateInfosString = updateInfosString
								+ "Update email success<br>";
					} else {
						addressSucess = 0;
						updateInfosString = updateInfosString
	...
	public void getInsuranceRates() {
		...
		managerDao.getInsuranceRates(managerFrame);
		managerFrame.getManageRatesPanel().getSaveInsuranceRateBtn()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnCancelInsuranceEdit()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnEditInsuranceRate()
				.setEnabled(true);
	}
	...
	public void getRentalRates() {
		...
		managerDao.getRentalRate(managerFrame);
		managerFrame.getManageRatesPanel().getRentalRateSaveEditBtn()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnCancelEditRental()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnEditRentalRate()
				.setEnabled(true);
	}
	...
	public void getAddnEquipRates() {
		...
		managerFrame.getManageRatesPanel().getBtnAddnEquipSaveBtn()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnAddnEquipCanel()
				.setEnabled(false);
		managerFrame.getManageRatesPanel().getBtnEditSelectedRate()
				.setEnabled(true);
	...
	private void assembleOtherRates(SuperRent superRent) {
		...
		managerFrame.getManageRatesPanel().setMembershipTxtField(
				String.valueOf(superRent.getMembershipFees()));
		managerFrame.getManageRatesPanel().setTaxField(
				String.valueOf(superRent.getTax()));
		managerFrame.getManageRatesPanel().setMemPointsTxt(
				String.valueOf(superRent.getMembershipPoints()));
		managerFrame.getManageRatesPanel().setFuelRateTxt(
				String.valueOf(superRent.getFuelRate()));
		managerFrame.getManageRatesPanel().setClubPointsPerDollarTxt(
				String.valueOf(superRent.getOneClubPoint()));
		managerFrame.getManageRatesPanel().setMinReedemablePointsTxt(
				superRent.getReedemablePoints());
	...
	private void saveOtherRates(SuperRent superRent) {
		...
		if (!managerFrame.getManageRatesPanel().getMembershipTxtField()
				.equalsIgnoreCase(null)) {
			superRent.setMembershipFees(Double.parseDouble(managerFrame
					.getManageRatesPanel().getMembershipTxtField()));
		}
		if (!managerFrame.getManageRatesPanel().getTaxField()
				.equalsIgnoreCase(null)) {
			superRent.setTax(Double.parseDouble(managerFrame
					.getManageRatesPanel().getTaxField()));
		}
		if (!managerFrame.getManageRatesPanel().getFuelRateTxt()
				.equalsIgnoreCase(null)) {
			superRent.setFuelRate(Double.parseDouble(managerFrame
					.getManageRatesPanel().getFuelRateTxt()));
		}
		if (!managerFrame.getManageRatesPanel().getClubPointsPerDollarTxt()
				.equalsIgnoreCase(null)) {
			superRent.setOneClubPoint(Double.parseDouble(managerFrame
					.getManageRatesPanel().getClubPointsPerDollarTxt()));
		}
		if (!managerFrame.getManageRatesPanel().getMinReedemablePointsTxt()
				.equalsIgnoreCase(null)) {
			superRent.setReedemablePoints(managerFrame.getManageRatesPanel()
					.getMinReedemablePointsTxt().toString());
		}
		if (!managerFrame.getManageRatesPanel().getMemPointsTxt()
				.equalsIgnoreCase(null)) {
			superRent.setMembershipPoints(Integer.parseInt(managerFrame
					.getManageRatesPanel().getMemPointsTxt()));
		}
	...
	private void checkValidation() {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				System.out
						.println(managerFrame.getAddVehiclePanel().getGroup());
				Problem validateAll = managerFrame.getAddVehiclePanel()
						.getGroup().performValidation();
				// System.out.println("validate all is  " + validateAll);
				if (validateAll == null) {
					managerFrame.getAddVehiclePanel().getBtnAdd()
							.setEnabled(true);
